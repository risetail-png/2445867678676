...
# 1ページ目ナビゲーションバー用: 「点検記録様式（そのＮ）」「データ記録様式（そのＮ）」
# 形式のページ見出しを汎用的に検出する（Ｎは "1" や "3-1" のような形式）
RE_FORM_SECTION = re.compile(
    r'(点検記録様式|データ記録様式)（その([0-9０-９]+(?:[-－ー][0-9０-９]+)?)）')

# 「データ記録様式（その３-１）」はこの汎用ナビからは除外し、径間ごとに個別処理する
# 「点検記録様式（その１）」は1ページ目自身（自己リンクになるため不要）
FORM_SECTION_EXCLUDE_SET = {
    ("データ記録様式", "3-1"),
    ("点検記録様式", "1"),
}
...
BTN_ROW_MAX_TOP    = 13   # オレンジ（1ページ目ナビ）ボタンの1行最大数
ROW_GAP = 2.0             # 段間の隙間(pt)

COLOR_NAV          = (180,  90,  20)   # 1ページ目ナビ（オレンジ）
COLOR_OUTLINE_NAV  = (130,  60,  10)
COLOR_ZERO         = (40,   44,  58)   # ZEROボタン（濃紺グレー）
COLOR_OUTLINE_ZERO = (10,   12,  20)

ZERO_BTN_W, ZERO_BTN_H = 38.0, 20.0    # ZEROボタンのサイズ(pt)（右上の空白いっぱいまで拡大）
ZERO_MARGIN_R, ZERO_MARGIN_T = 3.0, 2.0  # 右端・上端からの余白(pt)

NAV_BTN_H = 22.0          # 1ページ目ナビボタンの高さ(pt)（フリガナ行上の罫線まで拡張、2段対応）
NAV_MARGIN_T = 2.0        # 上端からの余白(pt)
...
def classify_pages(pdf_path):
    ...
    戻り値:
      ...
      form_first   : {(様式区分, 番号文字列): 最初に出現した page_idx}
                      例: {("点検記録様式","1"): 0, ("データ記録様式","3-2"): 21, ...}
    """
    ...
    for i, page in enumerate(reader.pages):
        ...
        if KEYWORD_PHOTO in text:
            photo.append(i)
        for m in RE_FORM_SECTION.finditer(text):
            prefix = m.group(1)
            num    = _normalize_text(m.group(2)).replace('－', '-').replace('ー', '-')
            key    = (prefix, num)
            if key not in form_first:
                form_first[key] = i
    return diag, photo, form_first


def _form_section_sort_key(key):
    """(様式区分, 番号) を自然順ソートするためのキー（例: "3-2" -> (3,2)）。"""
    prefix, num = key
    ...
    try:
        ...
        nums = (0,)
    prefix_order = 0 if prefix == "点検記録様式" else 1
    return (prefix_order, nums)


def _form_section_label(key):
    """ナビゲーションボタンに表示する短いラベルを組み立てる。"""
    prefix, num = key
    if prefix == "点検記録様式":
        return f"点{num}"
    return f"デ{num}"
...
def _place_button_row(pdf, page_idx, row_btn_list, x1, x2, y1, y2,
                      fill_color, outline_color, font_path, xobj_name):
    """1行分のボタン画像をページに描画しアノテーションを追加する（内部共通処理）。
    x1,x2,y1,y2 はすべて「表示上の座標系」（ビューアで見たままの向き）で受け取り、
...
def add_buttons_bottom(pdf, page_idx, btn_list, disp_w, disp_h,
    ...
    for r_i, row in enumerate(rows):
        ...
        y2 = y1 + BTN_H
        _place_button_row(pdf, page_idx, row, margin_l, margin_r, y1, y2,
                          fill_color, outline_color, font_path,
                          f'/{xobj_prefix}{page_idx}r{r_i}')


def add_buttons_top(pdf, page_idx, btn_list, disp_w, disp_h,
                    fill_color, outline_color, font_path, xobj_prefix):
    """ページ上端の余白帯に配置（1ページ目ナビ用）。
    BTN_ROW_MAX_TOP個超で下方向に折り返す（2段目が1段目の下）。"""
    margin_l = 64.0
    margin_r = disp_w - 48.0
    rows = [btn_list[i:i+BTN_ROW_MAX_TOP]
            for i in range(0, len(btn_list), BTN_ROW_MAX_TOP)]
    for r_i, row in enumerate(rows):
        y2 = disp_h - NAV_MARGIN_T - r_i * (NAV_BTN_H + ROW_GAP)
        y1 = y2 - NAV_BTN_H
        _place_button_row(pdf, page_idx, row, margin_l, margin_r, y1, y2,
                          fill_color, outline_color, font_path,
                          f'/{xobj_prefix}{page_idx}r{r_i}')
...
def add_zero_button(pdf, page_idx, target_idx, disp_w, disp_h, font_path,
                    xobj_prefix='ZeroBtn'):
    """右上に「ZERO」ボタン（1ページ目に戻る）を配置する。"""
    x2 = disp_w - ZERO_MARGIN_R
    x1 = x2 - ZERO_BTN_W
    y2 = disp_h - ZERO_MARGIN_T
    y1 = y2 - ZERO_BTN_H
    _place_button_row(pdf, page_idx, [("ZERO", target_idx)], x1, x2, y1, y2,
                      COLOR_ZERO, COLOR_OUTLINE_ZERO, font_path,
                      f'/{xobj_prefix}{page_idx}')
...
def process_one(input_path, output_path, font_path, log_cb):
    ...
    diag_pages, photo_pages, form_first = classify_pages(input_path)
    ...
    log_cb(f"  様式初出ページ: "
           f"{ {f'{k[0]}その{k[1]}': v+1 for k, v in sorted(form_first.items(), key=lambda item: _form_section_sort_key(item[0]))} }")
    ...
    # その３-１ 径間ごとの最初のページ {span -> first_page_idx}
    span_to_first_diag = {}
    ...
    # ── 1ページ目にナビゲーションバー（オレンジ）───────────────────────────
    nav_btn_list = []
    for key, pidx in form_first.items():
        if key in FORM_SECTION_EXCLUDE_SET:
            continue   # その３-１は下で径間ごとに個別追加、その１は自己リンクのため除外
        nav_btn_list.append((_form_section_label(key), pidx))
    for span in all_spans:
        if span in span_to_first_diag:
            label = f"デ3-1-{span}" if is_multi_span else "デ3-1"
            nav_btn_list.append((label, span_to_first_diag[span]))
    # 実際にPDF内で出現する順（ページ番号順）に並べる
    nav_btn_list.sort(key=lambda b: b[1])

    if nav_btn_list:
        dw, dh = get_display_size(pdf, first_page_idx)
        log_cb(f"  1ページ目 → ナビボタン{len(nav_btn_list)}個: "
               f"{[b[0] for b in nav_btn_list]}")
        add_buttons_top(pdf, first_page_idx, nav_btn_list, dw, dh,
                        COLOR_NAV, COLOR_OUTLINE_NAV, font_path, 'NavBtn')

    # ── 1ページ目以外の全ページ右上に「ZERO」ボタン（1ページ目へ戻る）───────
    total_pages = len(pdf.pages)
    for pidx in range(total_pages):
        if pidx == first_page_idx:
            continue
        dw, dh = get_display_size(pdf, pidx)
        add_zero_button(pdf, pidx, first_page_idx, dw, dh, font_path)
    log_cb(f"  ZEROボタン: {total_pages - 1} ページに追加（1ページ目へ戻る）")
...
class App(tk.Tk if not HAS_DND else TkinterDnD.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(legend, text="損傷写真→損傷図  ",
                 fg=self.SUBTEXT, bg=self.BG, font=("Yu Gothic UI", 9)).pack(side="left")
        tk.Label(legend, text="■", fg="#b45a14", bg=self.BG,
                 font=("Yu Gothic UI", 9)).pack(side="left")
        tk.Label(legend, text="1ページ目ナビ／ZEROで1ページ目へ  ／  出力は各PDFと同じフォルダに _linked.pdf で保存",
                 fg=self.SUBTEXT, bg=self.BG, font=("Yu Gothic UI", 9)).pack(side="left")
